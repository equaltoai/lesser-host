/**
 * Vite config for the M17 Account UI fixture page.
 *
 * Minimal config that serves ONLY `fixtures/m17-account.html`. The entry
 * point pre-loads a session store value and mocks `/api/v1/portal/me`, so
 * no backend proxy or API server is needed.
 *
...
export default defineConfig({
	...
	build: {
		...
		rollupOptions: {
			input: path.resolve(webDir, 'fixtures/m17-account.html'),
		},
	},
});
