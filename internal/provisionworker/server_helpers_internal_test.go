...
func TestProvisionWorker_HelperFunctions(t *testing.T) {
	...
	if !isRetryableAssumeRoleErr(errors.New("NoSuchEntity: role could not be found")) {
		t.Fatalf("expected not-found to be retryable")
	}
...
func TestAssumeInstanceRole_ValidationAndRetryableError(t *testing.T) {
	...
	s.sts = &fakeSTS{err: errors.New("NoSuchEntity: role has not propagated")}
