...
func (s *HostedGenesisSession) UpdateKeys() error {
	...
	if s.DeclarationCandidate != nil {
		...
		if candidate.InstanceSlug != s.InstanceSlug ||
			candidate.RegistrationID != s.RegistrationID ||
			!strings.EqualFold(candidate.AgentID, s.AgentID) ||
			candidate.ConversationID != s.ConversationID ||
			candidate.Model != s.Model ||
	...
	} else if s.CandidateRevision != 0 || strings.TrimSpace(s.CandidateHash) != "" || strings.TrimSpace(s.CandidatePhase) != "" {
