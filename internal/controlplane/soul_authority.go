...
func requireExplicitInstanceTrustAuthority(reg *models.SoulAgentRegistration, identity *models.SoulAgentIdentity) *apptheory.AppError {
	...
	if normalizeSoulAuthorityModel(reg.AuthorityModel) != models.SoulAuthorityModelInstanceTrust || normalizeSoulAuthorityModel(identity.AuthorityModel) != models.SoulAuthorityModelInstanceTrust {
		return &apptheory.AppError{Code: "app.conflict", Message: "hosted instance-trust authority is not recorded explicitly"}
	}
	return nil
}
