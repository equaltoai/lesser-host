...
test("hosted genesis AppTheory MicroVM deployed stages require NO credential-pair context (CDK-owned token)", () => {
  ...
  const template = synthesizeTemplate(
    "lab",
    { ...webLookupContext, ...hostedGenesisMicrovmContext("lab") },
    "TestLesserHostStackMissingMicrovmContext",
    { env: webStackEnv },
  );
  assert.ok(
  ...
  assert.throws(
    () =>
      synthesizeTemplate(
        "lab",
        { ...webLookupContext, hostedGenesisMicrovmDevTestOptOut: "true" },
        "TestLesserHostStackMicrovmOptOutLab",
        { env: webStackEnv },
      ),
...
test("hosted genesis AppTheory MicroVM deployed-stage wiring emits the digest as a custom-resource getAtt (not a literal raw token)", () => {
  ...
  const authorizerFn = findResourceEntries(
    template,
    "AWS::Lambda::Function",
  ).find(
    ([, fn]) =>
      fn.Properties?.FunctionName ===
      "lesser-host-lab-hosted-genesis-microvm-authorizer",
  );
  ...
  assert.ok(
    typeof digestValue === "object" &&
      JSON.stringify(digestValue).includes('"Fn::GetAtt"'),
  ...
  assert.ok(
    !templateJson.includes("sk-"),
    "no raw OpenAI key shape in synthesized template",
  );
  assert.ok(
    !templateJson.includes("sk_ant"),
    "no raw Anthropic key shape in synthesized template",
  );
  ...
  assert.ok(
    findResourceEntries(liveTemplate, "AWS::Lambda::Function").some(([, fn]) =>
      String(fn.Properties?.FunctionName ?? "").includes(
...
test("hosted genesis AppTheory MicroVM deployed-stage wiring uses AppTheory constructs with protected routes", () => {
  ...
  const connectors = findResourceEntries(
    template,
  ...
  assert.equal(
    connectors.length,
    0,
    "internetEgress synthesizes no AWS::Lambda::NetworkConnector resource",
  );
  ...
  assert.equal(
    imageProps.BaseImageVersion,
    "0",
    "expected deterministic base image version 0 (only available managed version)",
  );
  assert.ok(
    JSON.stringify(imageProps.BaseImageArn ?? "").includes(
      HOSTED_GENESIS_MICROVM_BASE_IMAGE_ARN,
    ),
  ...
  const egressRefs = imageProps.EgressNetworkConnectors as
    unknown[] | undefined;
  assert.ok(
    Array.isArray(egressRefs) && egressRefs.length === 1,
    "expected one egress connector ref on the image",
  );
  assert.ok(
    JSON.stringify(egressRefs?.[0] ?? "").includes(
  ...
  assert.equal(
    hooks?.Port,
    undefined,
    "expected NO Port on the no-hooks image (AWS rejects a Port with no hook groups; default port 8080 applies at runtime)",
  );
  assert.equal(
    hooks?.MicrovmImageHooks,
    undefined,
    "expected NO MicrovmImageHooks on the no-hooks image (build env cannot reach :8080 for /ready)",
  );
  assert.equal(
    hooks?.MicrovmHooks,
    undefined,
    "expected NO MicrovmHooks on the no-hooks image (endpoint-based architecture; controller POSTs to the runtime endpoint)",
  );
  ...
  const logging = imageProps.Logging as
    | {
  ...
  assert.ok(
    fs.existsSync(workloadDockerfile),
    "expected the hosted-genesis MicroVM workload asset buildDir to contain a generated Dockerfile",
  );
  ...
  const buildRole = findResourceEntries(template, "AWS::IAM::Role").find(
    ([, role]) =>
      String(role.Properties?.RoleName ?? "").includes(
        "hosted-genesis-microvm-image-build",
      ),
  );
  assert.ok(
    buildRole,
    "expected host-owned MicroVM image build role to synthesize",
  );
  ...
  assert.ok(
    buildServicePrincipals.some((p) => p === "lambda.amazonaws.com") &&
      !buildServicePrincipals.some(
        (p) => p === "microvms.lambda.amazonaws.com",
      ),
  ...
  const buildRolePolicies = findResourceEntries(
    template,
    "AWS::IAM::Policy",
  ).filter(([, policy]) => {
    ...
    return (
      ...
      roles.some(
        (role) =>
          ...
          typeof role === "object" &&
          "Ref" in role &&
          (role as { Ref?: string }).Ref === buildRoleLogicalId,
      )
    );
  });
  const buildRolePolicyJson = JSON.stringify(
  ...
  for (const action of [
    ...
    assert.ok(
      buildRolePolicyJson.includes(action),
      `expected MicroVM image build role to include ${action} for CloudWatch build logs`,
    );
  }
  assert.ok(
  ...
  const authorizerDigestEnv =
    authorizerEnv.HOSTED_GENESIS_MICROVM_AUTH_TOKEN_SHA256;
  assert.ok(
    typeof authorizerDigestEnv === "object" &&
      JSON.stringify(authorizerDigestEnv).includes('"Fn::GetAtt"'),
    "expected authorizer token digest env to be a CloudFormation getAtt token from the CDK-owned custom resource",
  );
  assert.ok(
    JSON.stringify(authorizerEnv.APPTHEORY_MICROVM_AUTHORIZER_TOKEN_SHA256) ===
      JSON.stringify(authorizerDigestEnv),
  ...
  assert.equal(
    controllerEnv.APPTHEORY_MICROVM_CONTROLLER_AUTH_REQUIRED,
    "true",
  );
  assert.equal(controllerEnv.APPTHEORY_MICROVM_CONTROLLER_AUTH_DEFAULT, "deny");
  assert.equal(
    controllerEnv.APPTHEORY_MICROVM_CONTRACT_VERSION,
    "m16.microvm/v1",
  );
  assert.equal(
  ...
  assert.ok(
    controllerEnv.APPTHEORY_MICROVM_SESSION_REGISTRY_TABLE,
    "expected AppTheory registry table env",
  );
  assert.ok(
    controllerEnv.APPTHEORY_MICROVM_INGRESS_NETWORK_CONNECTOR_REFS,
    "expected ingress connector refs env",
  );
  assert.ok(
    JSON.stringify(
      controllerEnv.APPTHEORY_MICROVM_INGRESS_NETWORK_CONNECTOR_REFS,
  ...
  assert.ok(
    controllerEnv.APPTHEORY_MICROVM_EGRESS_NETWORK_CONNECTOR_REFS,
    "expected egress connector refs env",
  );
  assert.ok(
    controllerEnv.APPTHEORY_MICROVM_SHELL_INGRESS_NETWORK_CONNECTOR_REF,
    "expected shell ingress connector ref env",
  );
  assert.ok(
    typeof controllerEnv.HOSTED_GENESIS_MICROVM_AUTH_TOKEN_SHA256 ===
      "object" &&
      JSON.stringify(
        controllerEnv.HOSTED_GENESIS_MICROVM_AUTH_TOKEN_SHA256,
      ).includes('"Fn::GetAtt"'),
    "expected controller token digest env to be a CloudFormation getAtt token from the CDK-owned custom resource",
  );
  assert.ok(
    controllerEnv.STATE_TABLE_NAME,
    "expected Host state table env for HostedGenesisSession reconstruction",
  );
  assert.ok(
  ...
  const routes = findResources(template, "AWS::ApiGatewayV2::Route").filter(
    (route) => String(route.RouteKey ?? "").includes("/microvms"),
  );
  assert.deepEqual(
  ...
  for (const route of routes) {
    assert.equal(
      ...
      "CUSTOM",
      `route ${route.RouteKey} must use the fail-closed authorizer`,
    );
    assert.ok(
      route.AuthorizerId,
      `route ${route.RouteKey} must attach authorizer id`,
    );
  }

  const sessionTable = findResourceEntries(
    template,
    "AWS::DynamoDB::Table",
  ).find(([, table]) =>
    JSON.stringify(table.Properties?.TableName).includes(
      "hosted-genesis-microvm-sessions",
    ),
  );
  assert.ok(
    sessionTable,
    "expected AppTheory controller-owned session registry table",
  );
  const ttl = sessionTable[1].Properties?.TimeToLiveSpecification as
    { AttributeName?: unknown; Enabled?: unknown } | undefined;
  assert.deepEqual(ttl, { AttributeName: "ttl", Enabled: true });

  const controllerRoleRef = controllerFn[1].Properties?.Role as
    { "Fn::GetAtt"?: unknown[] } | undefined;
  assert.ok(
    controllerRoleRef && Array.isArray(controllerRoleRef["Fn::GetAtt"]),
    "expected controller Lambda role reference",
  );
  const controllerRoleLogicalId = String(
    controllerRoleRef["Fn::GetAtt"][0] ?? "",
  );
  assert.ok(
    controllerRoleLogicalId,
    "expected controller Lambda role logical id",
  );
  const controllerPolicies = findResourceEntries(
    template,
    "AWS::IAM::Policy",
  ).filter(([, policy]) => {
    const roles = policy.Properties?.Roles;
    return (
      ...
      roles.some(
        (role) =>
          ...
          typeof role === "object" &&
          "Ref" in role &&
          (role as { Ref?: string }).Ref === controllerRoleLogicalId,
      )
    );
  });
  const controllerPolicyJson = JSON.stringify(
    controllerPolicies.map(([, policy]) => policy.Properties ?? {}),
  );
  for (const action of [
    "lambda:RunMicrovm",
    "lambda:GetMicrovm",
    ...
    assert.ok(
      controllerPolicyJson.includes(action),
      `expected controller IAM to include ${action}`,
    );
  }
  ...
  const hasWildcardMicrovmControlGrant = controllerPolicies.some(
    ([, policy]) => {
      const statements = policy.Properties?.PolicyDocument as
      ...
      return statements.Statement.some((statement) => {
        ...
        const resources =
          typeof typed.Resource === "string"
            ? [typed.Resource]
            : Array.isArray(typed.Resource)
              ? typed.Resource.filter(
        ...
        return (
          ...
          wildcardMicrovmActions.every((action) =>
  ...
  const provisioned = aliases.some(([, alias]) => {
    const cfg = (alias.Properties ?? {}) as {
      ProvisionedConcurrencyConfig?: {
        ProvisionedConcurrentExecutions?: unknown;
      };
    };
    return (
      cfg.ProvisionedConcurrencyConfig &&
      Number(cfg.ProvisionedConcurrencyConfig.ProvisionedConcurrentExecutions) >
        0
    );
  });
  assert.ok(
    provisioned,
    "expected the MicroVM controller Lambda to carry provisioned concurrency (currentVersionOptions)",
  );
});
...
test("P52 H1.5 corrective: host-owned MicroVM execution role propagates to RunMicrovm via the controller", () => {
  ...
  const executionRole = roles.find(([, role]) =>
    String(role.Properties?.RoleName ?? "").includes(
      "hosted-genesis-microvm-execution",
    ),
  );
  assert.ok(
    executionRole,
    "expected host-owned MicroVM execution role to synthesize",
  );
  const execRoleLogicalId = executionRole[0];
  const buildRole = roles.find(([, role]) =>
    String(role.Properties?.RoleName ?? "").includes(
      "hosted-genesis-microvm-image-build",
    ),
  );
  ...
  assert.ok(
    execRoleArnEnv,
    "expected APPTHEORY_MICROVM_EXECUTION_ROLE_ARN on the controller Lambda",
  );
  const execRoleArnStr = JSON.stringify(execRoleArnEnv);
  assert.ok(
    execRoleArnStr.includes('"Fn::GetAtt"') &&
  ...
  assert.ok(
    controllerPolicyJson.includes('"Fn::GetAtt"') &&
      controllerPolicyJson.includes(execRoleLogicalId),
    ...
    return (
      ...
      rolesList.some(
        (role) =>
          ...
          typeof role === "object" &&
          "Ref" in role &&
          (role as { Ref?: string }).Ref === execRoleLogicalId,
      )
    );
  });
  const execRolePolicyJson = JSON.stringify(
    execRolePolicies.map(([, policy]) => policy.Properties ?? {}),
  );
  for (const action of [
    "dynamodb:GetItem",
    "dynamodb:PutItem",
    ...
    assert.ok(
      ...
      `expected execution role to include ${action}`,
    );
  }
  assert.ok(
    execRolePolicyJson.includes("/lesser-host/api/openai/service") &&
      execRolePolicyJson.includes("/lesser-host/api/claude"),
  ...
  assert.ok(
    execRolePolicyJson.includes('"kms:ViaService"') &&
      execRolePolicyJson.includes("ssm."),
    "expected kms:Decrypt constrained via SSM ViaService",
  );
  const forbiddenExecActions = [
    "iam:PassRole",
    "iam:CreateRole",
    "iam:*",
    "ssm:PutParameter",
  ...
  for (const forbidden of forbiddenExecActions) {
    assert.ok(
      !execRolePolicyJson.includes(`"${forbidden}"`),
      `execution role must not grant ${forbidden}`,
    );
  }
  // No wildcard SSM resource and no wildcard DynamoDB resource on the execution role.
  assert.ok(
    !execRolePolicyJson.includes('"ssm:GetParameter","Resource":["*"]'),
  ...
  assert.ok(
    stateTableVar,
    "expected STATE_TABLE_NAME in the MicroVM image environment",
  );
  assert.ok(
...
test("P52 #873: CDK-owned auth-token custom resource generates the bearer token + writes only the digest + param name to CloudFormation", () => {
  ...
  const customResources = findResourceEntries(
    template,
  ...
  const provisionerFn = findResourceEntries(
    ...
    ([, fn]) =>
      fn.Properties?.FunctionName ===
      "lab-hosted-genesis-microvm-auth-token-provisioner",
  );
  assert.ok(
    provisionerFn,
  ...
  const provisionerCodeJson = JSON.stringify(
    provisionerFn[1].Properties?.Code ?? {},
  );
  assert.ok(
    provisionerCodeJson.includes("randomBytes"),
    "expected the inline handler to generate the token with crypto.randomBytes",
  );
  assert.ok(
    !provisionerCodeJson.includes("sk-"),
    "no raw OpenAI key shape in the provisioner handler",
  );
  assert.ok(
    !provisionerCodeJson.includes("sk_ant"),
    "no raw Anthropic key shape in the provisioner handler",
  );
  ...
  const provisionerRoleRef = provisionerFn[1].Properties?.Role as
    { "Fn::GetAtt"?: unknown[] } | undefined;
  assert.ok(
    provisionerRoleRef && Array.isArray(provisionerRoleRef["Fn::GetAtt"]),
    "expected provisioner Lambda role reference",
  );
  const provisionerRoleLogicalId = String(
    provisionerRoleRef["Fn::GetAtt"][0] ?? "",
  );
  const provisionerPolicies = findResourceEntries(
    template,
    "AWS::IAM::Policy",
  ).filter(([, policy]) => {
    const roles = policy.Properties?.Roles;
    return (
      ...
      roles.some(
        (role) =>
          ...
          typeof role === "object" &&
          "Ref" in role &&
          (role as { Ref?: string }).Ref === provisionerRoleLogicalId,
      )
    );
  });
  const provisionerPolicyJson = JSON.stringify(
    provisionerPolicies.map(([, policy]) => policy.Properties ?? {}),
  );
  for (const action of [
    "ssm:PutParameter",
    "ssm:GetParameter",
    ...
    assert.ok(
      provisionerPolicyJson.includes(action),
      `expected provisioner IAM to include ${action} for the auth-token secret`,
    );
  }
  assert.ok(
    provisionerPolicyJson.includes(
      "/lesser-host/lab/hosted-genesis/microvm/auth-token",
    ),
    "expected provisioner ssm grants scoped to the deterministic auth-token SSM parameter",
  );
  assert.ok(
    provisionerPolicyJson.includes('"kms:ViaService"') &&
      provisionerPolicyJson.includes("ssm."),
  ...
  for (const forbidden of [
    "iam:PassRole",
    "lambda:RunMicrovm",
    'ssm:PutParameter","Resource":["*"]',
  ]) {
    assert.ok(
      !provisionerPolicyJson.includes(forbidden),
      `provisioner Lambda must NOT carry ${forbidden}`,
    );
  }
  ...
  assert.ok(
    !fullTemplateJson.includes(
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    ),
    "no literal test sha256 digest in the synthesized template",
  );
  assert.ok(
    !fullTemplateJson.includes("Bearer "),
    "no raw bearer token shape in the synthesized template",
  );
...
test("P52 H1.5: control-plane Lambda receives HTTP dispatch env + SSM auth-token grant (no MicroVM IAM, no registry writes)", () => {
  ...
  const controlPlaneRoleRef = controlPlaneFn[1].Properties?.Role as
    { "Fn::GetAtt"?: unknown[] } | undefined;
  assert.ok(
    controlPlaneRoleRef && Array.isArray(controlPlaneRoleRef["Fn::GetAtt"]),
    "expected control-plane Lambda role reference",
  );
  const controlPlaneRoleLogicalId = String(
    controlPlaneRoleRef["Fn::GetAtt"][0] ?? "",
  );
  assert.ok(
    controlPlaneRoleLogicalId,
    "expected control-plane Lambda role logical id",
  );
    ...
    return (
      ...
      roles.some(
        (role) =>
          ...
          typeof role === "object" &&
          "Ref" in role &&
          (role as { Ref?: string }).Ref === controlPlaneRoleLogicalId,
      )
    );
  });
  const controlPlanePolicyJson = JSON.stringify(
    controlPlanePolicies.map(([, policy]) => policy.Properties ?? {}),
  );
  assert.ok(
    controlPlanePolicyJson.includes("ssm:GetParameter"),
    "expected control-plane IAM to include ssm:GetParameter for the auth-token secret",
  );
  assert.ok(
    controlPlanePolicyJson.includes("kms:Decrypt"),
    "expected control-plane IAM to include kms:Decrypt for the auth-token SecureString",
  );
  assert.ok(
    controlPlanePolicyJson.includes(
      "/lesser-host/lab/hosted-genesis/microvm/auth-token",
    ),
