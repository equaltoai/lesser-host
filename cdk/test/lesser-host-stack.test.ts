...
import { shouldUseLocalWebBundling } from "../lib/lesser-host-stack";
import {
...
// P52 H1 MAI-1 split: the hosted-genesis MicroVM tests were extracted into
// ./lesser-host-stack-microvm.test.ts and the shared synth/lookup helpers
// both files depend on were factored into ./_lesser-host-test-helpers so
// neither test file duplicates them or exceeds the gov-infra TS/JS 2000-line
// file budget. Coverage and assertions are unchanged.
import {
  SynthesizedTemplate,
  ...
  findResources,
  lambdaEnvironment,
  runDependencyCycleValidator,
  synthTemplate,
  synthesizeTemplate,
  synthTemplateForStage,
  synthTemplateForStageWithCdkJsonContext,
...
function parseAccessLogFormat(
  stage: Record<string, unknown>,
): Record<string, unknown> {
  const accessLogSettings = stage.AccessLogSettings;
  assert.ok(
    accessLogSettings && typeof accessLogSettings === "object",
    "expected stage access log settings",
  );
...
test("state table exposes the active update recovery index", () => {
  ...
  const matchingTable = tables.find((table) => {
    ...
    return indexes.some((index) => {
      ...
      return (
        name === "gsi2" &&
        keySchema.some(
          (key) => key.AttributeName === "gsi2PK" && key.KeyType === "HASH",
        ) &&
        keySchema.some(
          (key) => key.AttributeName === "gsi2SK" && key.KeyType === "RANGE",
        )
      );
    });
  });
  ...
  assert.ok(
    matchingTable,
    "expected state table gsi2 for active update recovery",
  );
});
...
test("stack schedules the managed update sweep every five minutes", () => {
  ...
  const matchingRule = rules.find((rule) => {
    ...
    return (
      rule.Name === "lesser-host-lab-update-sweep" &&
...
test("stages add unique SES inbound rules to the shared activated rule set", () => {
  for (const { stage, recipient, ruleName } of [
    {
      stage: "lab",
      recipient: LAB_SOUL_EMAIL_INBOUND_DOMAIN,
      ruleName: "lesser-host-lab-ingress",
    },
    {
      stage: "live",
      recipient: LIVE_SOUL_EMAIL_INBOUND_DOMAIN,
      ruleName: "lesser-host-live-ingress",
    },
  ]) {
    ...
    assert.equal(
      rules.length,
      1,
      `expected exactly one SES receipt rule in ${stage}`,
    );
    ...
    assert.ok(
      ...
      `expected ${stage} receipt rule properties`,
    );
    ...
    const inboundBucket = findResourceEntries(template, "AWS::S3::Bucket").find(
      ([, bucket]) => {
        const bucketName = JSON.stringify(bucket.Properties?.BucketName);
        return (
          bucketName.includes(`lesser-host-${stage}-`) &&
          bucketName.includes("-inbound-email")
        );
      },
    );
    assert.ok(inboundBucket, `expected ${stage} inbound email bucket`);
    const s3Action = (actions[0] as { S3Action?: Record<string, unknown> })
      .S3Action;
    ...
    const emailIngressFn = findResourceEntries(
      template,
      "AWS::Lambda::Function",
    ).find(
      ([, fn]) =>
...
test("stage-specific inbound bridge domain defaults are explicit", () => {
  assert.equal(
    defaultSoulEmailInboundDomainForStage("lab"),
    LAB_SOUL_EMAIL_INBOUND_DOMAIN,
  );
  assert.equal(
    defaultSoulEmailInboundDomainForStage(" live "),
    LIVE_SOUL_EMAIL_INBOUND_DOMAIN,
  );
  assert.equal(
    defaultSoulEmailInboundDomainForStage("preview"),
    LAB_SOUL_EMAIL_INBOUND_DOMAIN,
  );
});
...
test("control-plane HTTP API access logs do not persist raw request paths", () => {
  ...
  const controlPlaneStage = stages.find(
    (stage) => stage.StageName === "$default",
  );
...
test("private mint-conversation instance read route has a templated HTTP API route key", () => {
  ...
  assert.ok(
    matching,
    "expected private mint-conversation single-read route to have a non-raw route key",
  );
});

function findLambdaByFunctionName(
...
function findBucketByLogicalIdPrefix(
  ...
  const entry = findResourceEntries(template, "AWS::S3::Bucket").find(
    ([logicalId]) => logicalId.startsWith(logicalIdPrefix),
  );
  assert.ok(
    entry,
    `expected S3 bucket logical id starting with ${logicalIdPrefix}`,
  );
  return entry[1].Properties ?? {};
}

function bucketLifecycleRules(
  bucket: Record<string, unknown>,
): Array<Record<string, unknown>> {
  const lifecycle = bucket.LifecycleConfiguration;
  assert.ok(
    lifecycle && typeof lifecycle === "object",
    "expected bucket lifecycle configuration",
  );
...
function lifecycleRuleExpiresCurrentObjects(
  rule: Record<string, unknown>,
): boolean {
  return (
    "ExpirationInDays" in rule ||
    "ExpirationDate" in rule ||
    "ExpiredObjectDeleteMarker" in rule
  );
}
...
function expiringLifecycleRulesMatchingKey(
  ...
  return rules.filter(lifecycleRuleExpiresCurrentObjects).filter((rule) => {
...
test("trust api receives ENS gateway runtime configuration separate from soul registry config", () => {
  const template = synthTemplateForStage("live", {
    ...
    soulRegistryContractAddressLive:
  ...
  assert.equal(
    env.ENS_GATEWAY_RESOLVER_ADDRESS,
...
test("ENS gateway lab and live context selection uses stage-owned resolver and chain values", () => {
  ...
  assert.equal(
    ensGatewayContextForStage(scope, "lab", "ensGatewayEnabled"),
    "true",
  );
  assert.equal(
    ensGatewayContextForStage(scope, "live", "ensGatewayEnabled"),
    "true",
  );
...
test("provision runner cannot assume the organization vending role", () => {
  const orgVendingRoleArn =
    "arn:aws:iam::123456789012:role/lesser-host-org-vending";
  const template = synthTemplateWithContext({
    managedOrgVendingRoleArn: orgVendingRoleArn,
  });
  ...
  for (const [logicalId, resource] of Object.entries(resources)) {
    ...
    assert.ok(
      ...
      `expected ${logicalId} to have policy statements`,
    );
    for (const statement of statements) {
      const resourceValue =
        statement && typeof statement === "object"
          ...
          : undefined;
      const resourcesValue = Array.isArray(resourceValue)
        ? resourceValue
        : [resourceValue];
      assert.ok(
...
test("placeholder organization vending role context is omitted from deployable wiring", () => {
  const placeholderOrgVendingRoleArn =
    "arn:aws:iam::<YOUR_ORG_ACCOUNT_ID>:role/lesser-host-org-vending";
  const template = synthTemplateWithContext({
    managedOrgVendingRoleArn: placeholderOrgVendingRoleArn,
  });
  const provisionWorker = findLambdaByFunctionName(
    template,
    "provision-worker",
  );
  ...
  for (const [logicalId, resource] of findResourceEntries(
    template,
    "AWS::IAM::Policy",
  )) {
    assert.doesNotMatch(
...
test("malformed non-placeholder organization vending role context fails synthesis", () => {
  assert.throws(
    () =>
      synthTemplateWithContext({
        managedOrgVendingRoleArn: "not-an-iam-role-arn",
      }),
...
test("hosted genesis queue is non-authoritative MicroVM dispatch transport, consumed only by ai-worker", () => {
  const template = synthTemplate();
  ...
  assert.equal(
    queue[1].Properties?.VisibilityTimeout,
    180,
    "hosted genesis queue must cover ai-worker timeout",
  );
  assert.deepEqual(
  ...
  assert.ok(
    ...
    "control plane must receive hosted genesis queue url for enqueue-only dispatch handoff",
  );
  assert.ok(
    "HOSTED_GENESIS_QUEUE_URL" in aiWorkerEnv,
    "ai worker must receive hosted genesis queue url",
  );
  assert.equal(
    aiWorker?.Timeout,
    120,
    "ai worker needs durable async LLM work timeout headroom",
  );

  const controlPlaneEntry = findLambdaEntryByFunctionName(
    template,
    "control-plane-api",
  );
  assert.ok(controlPlaneEntry, "expected control-plane Lambda entry");
  const controlPlaneRoleLogicalId = firstLogicalReference(
    controlPlaneEntry[1].Properties?.Role,
  );
  assert.ok(controlPlaneRoleLogicalId, "expected control-plane role reference");
  const controlPlanePolicies = findResourceEntries(
    template,
    "AWS::IAM::Policy",
  ).filter(([, policy]) =>
    JSON.stringify(policy.Properties?.Roles ?? "").includes(
      controlPlaneRoleLogicalId,
    ),
  );
  const controlPlanePolicyText = JSON.stringify(controlPlanePolicies);
  assert.ok(
    controlPlanePolicyText.includes(queue[0]) &&
      controlPlanePolicyText.includes("sqs:SendMessage"),
    "control-plane role must have hosted genesis SQS SendMessage authority for enqueue-only handoff",
  );
  assert.ok(
    !controlPlanePolicyText.includes("sqs:ReceiveMessage") &&
      !controlPlanePolicyText.includes("sqs:DeleteMessage"),
    "control-plane role must not have hosted genesis SQS consume authority",
  );

  const mappings = findResources(template, "AWS::Lambda::EventSourceMapping");
  const mapping = mappings.find(
    (entry) =>
      JSON.stringify(entry).includes(queue[0]) &&
      JSON.stringify(entry).includes("AiWorker"),
  );
  assert.ok(mapping, "expected hosted genesis queue event source on ai-worker");
  assert.equal(
    mapping.BatchSize,
    1,
    "hosted genesis jobs must process one conversation turn at a time",
  );
  assert.deepEqual(
    mapping.FunctionResponseTypes,
    ["ReportBatchItemFailures"],
    "hosted genesis handler failures must remain retryable",
  );
  const controlPlaneMapping = mappings.find(
    (entry) =>
      JSON.stringify(entry).includes(queue[0]) &&
      JSON.stringify(entry).includes("ControlPlaneApi"),
  );
  assert.ok(
    !controlPlaneMapping,
    "control-plane Lambda must not consume hosted genesis queue",
  );
});
...
function hostedGenesisTemplateGuardPath(): string {
  return join(
    ...
    "..",
    "scripts",
    "validate-hosted-genesis-template.mjs",
  );
}
...
function deployTemplatePlaceholderGuardPath(): string {
  return join(
    ...
    "..",
    "scripts",
    "validate-deploy-template-placeholders.mjs",
  );
}
...
function liveDomainTemplateGuardPath(): string {
  return join(
    ...
    "..",
    "scripts",
    "validate-live-domain-template.mjs",
  );
}

function writeTemplateGuardFixture(template: SynthesizedTemplate): {
  path: string;
  cleanup: () => void;
} {
...
function runHostedGenesisTemplateGuard(templatePath: string): {
  status: number | null;
  stderr: string;
} {
  const result = spawnSync(
    process.execPath,
    [hostedGenesisTemplateGuardPath(), templatePath],
    {
      encoding: "utf8",
    },
  );
...
function runDeployTemplatePlaceholderGuard(templatePath: string): {
  status: number | null;
  stderr: string;
} {
  const result = spawnSync(
    process.execPath,
    [deployTemplatePlaceholderGuardPath(), templatePath],
    {
      encoding: "utf8",
    },
  );
...
function runLiveDomainTemplateGuard(
  stage: string,
  templatePath: string,
): { status: number | null; stderr: string } {
  const result = spawnSync(
    process.execPath,
    [liveDomainTemplateGuardPath(), stage, templatePath],
    {
      encoding: "utf8",
    },
  );
...
function stalePreM2TemplateFixture(
  template: SynthesizedTemplate,
): SynthesizedTemplate {
  ...
  for (const resource of Object.values(stale.Resources ?? {})) {
    ...
    const variables = (
      resource.Properties?.Environment as
        { Variables?: Record<string, unknown> } | undefined
    )?.Variables;
...
function staleQueueAuthorityTemplateFixture(
  template: SynthesizedTemplate,
): SynthesizedTemplate {
  const stale = JSON.parse(JSON.stringify(template)) as SynthesizedTemplate;
  const hostedQueue = findResourceEntries(stale, "AWS::SQS::Queue").find(
    ([, resource]) =>
      resource.Properties?.QueueName === "lesser-host-lab-hosted-genesis-queue",
  );
  assert.ok(hostedQueue, "expected hosted genesis queue fixture target");
  const controlPlane = findLambdaEntryByFunctionName(
    stale,
    "control-plane-api",
  );
  ...
  stale.Resources = {
    ...(stale.Resources ?? {}),
    HostedGenesisControlPlaneQueueAuthorityMapping: {
      Type: "AWS::Lambda::EventSourceMapping",
      Properties: {
        EventSourceArn: { "Fn::GetAtt": [hostedQueue[0], "Arn"] },
        FunctionName: { Ref: controlPlane[0] },
        BatchSize: 1,
      },
    },
  };
...
function staleHostedGenesisRetryTemplateFixture(
  template: SynthesizedTemplate,
): SynthesizedTemplate {
  const stale = JSON.parse(JSON.stringify(template)) as SynthesizedTemplate;
  const hostedQueue = findResourceEntries(stale, "AWS::SQS::Queue").find(
  ...
  assert.ok(hostedQueue, "expected hosted genesis queue fixture target");
  const aiWorker = findLambdaEntryByFunctionName(stale, "ai-worker");
  assert.ok(aiWorker, "expected ai-worker fixture target");
  const mapping = findResourceEntries(
    stale,
    "AWS::Lambda::EventSourceMapping",
  ).find(
    ([, resource]) =>
      JSON.stringify(resource.Properties?.EventSourceArn ?? {}).includes(
        hostedQueue[0],
      ) &&
      JSON.stringify(resource.Properties?.FunctionName ?? {}).includes(
        aiWorker[0],
      ),
  );
  assert.ok(mapping, "expected hosted genesis event source fixture target");
  delete mapping[1].Properties?.FunctionResponseTypes;
  return stale;
}

function withPlaceholderIamPolicy(
  template: SynthesizedTemplate,
): SynthesizedTemplate {
...
function withPlaceholderManagedOrgVendingEnv(
  template: SynthesizedTemplate,
): SynthesizedTemplate {
  const copy = JSON.parse(JSON.stringify(template)) as SynthesizedTemplate;
  const commWorker = findResourceEntries(copy, "AWS::Lambda::Function").find(
    ([, resource]) =>
      resource.Properties?.FunctionName === "lesser-host-lab-comm-worker",
  );
  assert.ok(commWorker, "expected comm worker fixture target");
  const environment = (commWorker[1].Properties?.Environment ?? {}) as Record<
    string,
    unknown
  >;
...
function withoutLiveDomainBindings(
  template: SynthesizedTemplate,
): SynthesizedTemplate {
  const copy = JSON.parse(JSON.stringify(template)) as SynthesizedTemplate;
  for (const [, resource] of findResourceEntries(
    copy,
    "AWS::CloudFront::Distribution",
  )) {
    const config = (resource.Properties?.DistributionConfig ?? {}) as Record<
      string,
      unknown
    >;
  ...
  for (const [logicalId, resource] of findResourceEntries(
    ...
    if (
      logicalId.startsWith("WebAlias") ||
      normalizedDnsName(resource.Properties?.Name) === "lesser.host"
    ) {
  ...
  for (const [, resource] of findResourceEntries(
    ...
    if (variables.PUBLIC_BASE_URL !== undefined)
      variables.PUBLIC_BASE_URL = "https://d111111abcdef8.cloudfront.net";
  }
  (copy.Outputs!.WebUrl as { Value?: unknown }).Value =
...
test("hosted genesis deploy preflight validator accepts the synthesized M4 template", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /HostedGenesisSession as user-visible authority, HostedGenesisQueue as non-authoritative MicroVM dispatch\/backfill transport/,
    );
  } finally {
    fixture.cleanup();
  }
...
test("hosted genesis deploy preflight validator rejects stale pre-M2 templates", () => {
  const fixture = writeTemplateGuardFixture(
    stalePreM2TemplateFixture(synthTemplate()),
  );
  try {
    const result = runHostedGenesisTemplateGuard(fixture.path);
    assert.notEqual(
      result.status,
      0,
      "expected stale pre-M2 template to fail hosted genesis guard",
    );
...
test("hosted genesis deploy preflight validator rejects stale queue-authority templates", () => {
  const fixture = writeTemplateGuardFixture(
    staleQueueAuthorityTemplateFixture(synthTemplate()),
  );
  try {
    const result = runHostedGenesisTemplateGuard(fixture.path);
    assert.notEqual(
      result.status,
      0,
      "expected stale queue-authority template to fail hosted genesis guard",
    );
    assert.match(
      result.stderr,
      /control-plane Lambda must not consume HOSTED_GENESIS_QUEUE_URL/,
    );
  } finally {
    fixture.cleanup();
  }
...
test("hosted genesis deploy preflight validator rejects mappings that acknowledge handler failures", () => {
  const fixture = writeTemplateGuardFixture(
    staleHostedGenesisRetryTemplateFixture(synthTemplate()),
  );
  try {
    const result = runHostedGenesisTemplateGuard(fixture.path);
    assert.notEqual(
      result.status,
      0,
      "expected missing ReportBatchItemFailures to fail hosted genesis guard",
    );
    assert.match(
      result.stderr,
...
test("hosted genesis deploy preflight validator rejects live soul-disabled templates", () => {
  const fixture = writeTemplateGuardFixture(
    synthTemplateForStage("live", { soulEnabledLive: "false" }),
  );
  try {
    const result = runHostedGenesisTemplateGuard(fixture.path);
    assert.notEqual(
      result.status,
      0,
      "expected live soul-disabled template to fail hosted genesis guard",
    );
    assert.match(
      result.stderr,
      /live hosted genesis and soul search require SOUL_ENABLED=true/,
    );
...
test("deploy template placeholder guard accepts sanitized synthesized templates", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /no placeholder tokens in IAM resources or managed org vending env wiring/,
    );
  } finally {
    fixture.cleanup();
  }
...
test("deploy template placeholder guard rejects placeholder IAM policy resources", () => {
  const fixture = writeTemplateGuardFixture(
    withPlaceholderIamPolicy(synthTemplate()),
  );
  try {
    const result = runDeployTemplatePlaceholderGuard(fixture.path);
    assert.notEqual(
      result.status,
      0,
      "expected placeholder IAM policy to fail deploy template guard",
    );
    assert.match(
      result.stderr,
      /placeholder token found in deploy-critical template fields/,
    );
  } finally {
    fixture.cleanup();
  }
...
test("deploy template placeholder guard rejects placeholder org vending env wiring", () => {
  const fixture = writeTemplateGuardFixture(
    withPlaceholderManagedOrgVendingEnv(synthTemplate()),
  );
  try {
    const result = runDeployTemplatePlaceholderGuard(fixture.path);
    assert.notEqual(
      result.status,
      0,
      "expected placeholder env wiring to fail deploy template guard",
    );
...
test("live synthesizes lesser.host from AppTheory app.json web-domain config", () => {
  const template = synthTemplateForStage("live");
  const config = distributionConfig(template) as DistributionConfig & {
    Aliases?: unknown;
    ViewerCertificate?: Record<string, unknown>;
  };
  assert.deepEqual(config.Aliases, ["lesser.host"]);
  assert.ok(
    config.ViewerCertificate && "AcmCertificateArn" in config.ViewerCertificate,
  );

  const apexRecords = findResources(template, "AWS::Route53::RecordSet").filter(
    (record) => normalizedDnsName(record.Name) === "lesser.host",
  );
  assert.deepEqual(
    new Set(apexRecords.map((record) => record.Type)),
    new Set(["A", "AAAA"]),
  );
  ...
  assert.ok(
    ...
    "expected live lambdas to receive PUBLIC_BASE_URL",
  );
  assert.ok(publicBaseUrls.every((value) => value === "https://lesser.host"));
  assert.equal(
    (template.Outputs?.WebUrl as { Value?: unknown } | undefined)?.Value,
    "https://lesser.host",
  );
...
test("AppTheory app.json web-domain config fails closed when file or active stage config is missing", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        synthesizeTemplate("lab", {}, "TestMissingAppTheoryWebDomain", {
          env: webStackEnv,
          appConfigPath: missingPath,
        }),
          ...
          lesserHost: {
            webDomain: {
              live: {
                ...
                hostedZoneId: testWebHostedZoneId,
                hostedZoneName: "lesser.host",
              },
            },
          },
          cdk: {
    ...
    assert.throws(
      () =>
        synthesizeTemplate("lab", {}, "TestMissingStageAppTheoryWebDomain", {
          env: webStackEnv,
          appConfigPath: noLabStagePath,
        }),
...
test("AppTheory app.json web-domain config rejects empty hosted-zone id", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        synthesizeTemplate("lab", {}, "TestInvalidAppTheoryWebDomain", {
          env: webStackEnv,
          appConfigPath: invalidPath,
        }),
...
test("live cdk.json defaults keep hosted off-chain soul search enabled", () => {
  ...
  assert.deepEqual(
    new Set(Object.values(soulEnabledLambdas)),
    new Set(["true"]),
  );
  assert.ok(
    Object.keys(soulEnabledLambdas).some((logicalId) =>
      logicalId.includes("ControlPlaneApi"),
    ),
  );
  assert.ok(
    Object.keys(soulEnabledLambdas).some((logicalId) =>
      logicalId.includes("TrustApi"),
    ),
  );
...
test("live custom-domain deploy guard remains a backstop for broken domain resolution", () => {
  const fixture = writeTemplateGuardFixture(
    withoutLiveDomainBindings(synthTemplateForStage("live")),
  );
  try {
    const result = runLiveDomainTemplateGuard("live", fixture.path);
    assert.notEqual(
      result.status,
      0,
      "expected broken live template to fail live custom-domain guard",
    );
    assert.doesNotMatch(result.stderr, /cdk\/cdk\.context\.local\.json/);
    assert.match(
      result.stderr,
      /domain resolution is broken or AppTheory app\.json web-domain config is invalid or unavailable/,
    );
...
test("live custom-domain deploy guard accepts the AppTheory app.json hosted zone", () => {
  const template = synthTemplateForStage("live");
  assert.ok(
    findResources(template, "AWS::Route53::RecordSet").some(
      (record) => record.HostedZoneId === testWebHostedZoneId,
    ),
...
test("live custom-domain deploy guard skips lab templates intentionally", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /only live is required to preserve lesser\.host/,
    );
  } finally {
    fixture.cleanup();
  }
...
test("LH-27 htmlStore lifecycle does not expire static hydration sidecars", () => {
  const template = synthTemplate();
  const htmlStoreBucket = findBucketByLogicalIdPrefix(
    template,
    "WebHtmlStoreBucket",
  );
  ...
  const expiringIsrRules = expiringLifecycleRulesMatchingKey(
    rules,
    "isr/index.html",
  );
  assert.equal(
...
function distributionConfig(template: SynthesizedTemplate): DistributionConfig {
  const distributions = findResources(
    template,
    "AWS::CloudFront::Distribution",
  );
  assert.equal(
  ...
  assert.ok(
    ...
    "distribution missing DistributionConfig",
  );
...
function webAclRules(template: SynthesizedTemplate): WebAclRule[] {
  ...
  assert.equal(
    webAcls.length,
    1,
    `expected exactly one WebACL, got ${webAcls.length}`,
  );
...
test("M0.13 distribution: preserves legacy logical id for in-place alias migration", () => {
  const distributions = findResourceEntries(
    synthTemplate(),
    "AWS::CloudFront::Distribution",
  );
  assert.equal(
...
test("M0.13 distribution: exactly one OAC-protected default SSR Lambda origin", () => {
  ...
  assert.ok(
    defaultTargetId,
    "distribution missing default behavior target origin",
  );
...
test("P51 M4 distribution: Lesser instance mint-conversation route uses JSON control-plane origin", () => {
  const config = distributionConfig(synthTemplate());
  const exactLesserPattern =
    "api/v1/soul/instance/agents/register/*/mint-conversation*";
  const behavior = (config.CacheBehaviors ?? []).find(
    (b) => b.PathPattern === exactLesserPattern,
  );
  assert.ok(
    behavior,
...
test("P49 deploy preflight: synthesized lab template has no CloudFormation dependency cycle", () => {
  const result = runDependencyCycleValidator(
    synthTemplate() as Record<string, unknown>,
  );
  assert.equal(
    result.status,
    0,
  ...
  assert.match(
    result.stdout,
    /CloudFormation dependency graph OK: no circular dependencies/,
  );
});
...
test("P49 deploy preflight: dependency-cycle validator fails on intrinsic cycles", () => {
  ...
  assert.equal(
    result.status,
    1,
    `expected dependency-cycle validator to fail\nstdout:\n${result.stdout}`,
  );
...
test("M0.13 WAF: allows no-User-Agent only for ENS /resolve CCIP-read", () => {
  ...
  const commonStatement = commonRule.Statement?.ManagedRuleGroupStatement as
    { RuleActionOverrides?: Array<Record<string, unknown>> } | undefined;
  ...
  const customRule = rules.find(
    (rule) => rule.Name === "BlockNoUserAgentExceptResolve",
  );
  assert.ok(customRule, "expected custom no-User-Agent block rule");
  assert.deepEqual(
    customRule.Action,
    { Block: {} },
    "custom no-User-Agent rule must block",
  );
  assert.equal(
    customRule.Priority,
    1,
    "custom no-User-Agent rule must run immediately after CommonRuleSet",
  );

  const andStatement = customRule.Statement?.AndStatement as
  ...
  assert.equal(
    statements.length,
    2,
    "custom no-User-Agent rule must have exactly label and path predicates",
  );

  const labelPredicate = statements.find(
    (statement) => statement.LabelMatchStatement,
  );
  assert.deepEqual(
    labelPredicate,
    {
...
function findCspByPolicyName(
  ...
  const match = policies.find((p) => {
    const config = (p as Record<string, unknown>)
      .ResponseHeadersPolicyConfig as { Name?: unknown } | undefined;
    const name = config?.Name;
    return typeof name === "string" && name.endsWith(nameSuffix);
  }) as Record<string, unknown> | undefined;
  assert.ok(
    match,
    `expected a ResponseHeadersPolicy with name ending in ${nameSuffix}`,
  );
...
test("M0.14 CSP integrity: webCsp + safeAppCsp reject every CSP relaxation vector", () => {
  ...
  const policies = [
    EXPECTED_WEB_CSP,
    EXPECTED_SAFE_APP_CSP,
    findCspByPolicyName(template, "-web-security"),
    findCspByPolicyName(template, "-safe-app-security"),
  ];
  ...
  for (const csp of [renderedWeb, renderedSafeApp]) {
    ...
    assert.ok(
      !csp.includes("'unsafe-hashes'"),
      "CSP must not contain 'unsafe-hashes'",
    );
    assert.ok(!/nonce-/.test(csp), "CSP must not contain nonce-* relaxations");
    assert.ok(
      !/sha256-/.test(csp),
      "CSP must not contain sha256-* relaxations",
    );
    assert.ok(
      !/sha384-/.test(csp),
      "CSP must not contain sha384-* relaxations",
    );
    assert.ok(
      !/sha512-/.test(csp),
      "CSP must not contain sha512-* relaxations",
    );
  ...
  assert.ok(
    /frame-ancestors https:\/\/safe.global https:\/\/\*\.safe\.global/.test(
      renderedSafeApp,
    ),
