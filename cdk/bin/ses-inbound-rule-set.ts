...
const account =
	...
	(app.node.tryGetContext('sesInboundRuleSetAccountId') as string | undefined) ||
...
const region =
	...
	(app.node.tryGetContext('sesInboundRuleSetRegion') as string | undefined) ||
	regionFromProfile ||
	undefined;
const env = account || region ? { account, region } : undefined;

const stackName =
	(app.node.tryGetContext('sesInboundRuleSetStackName') as string | undefined) ??
	'lesser-host-ses-inbound-rule-set';

new SesInboundRuleSetStack(app, stackName, { env });
