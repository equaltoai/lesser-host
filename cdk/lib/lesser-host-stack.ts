...
export class LesserHostStack extends cdk.Stack {
	...
	constructor(scope: Construct, id: string, props: LesserHostStackProps) {
		...
		const provisionRunnerBuild = [
			...
			'LESSER_UP_STAGE=""',
			'if [ -n "${LESSER_STAGE:-}" ]; then',
			'  export STAGE="$LESSER_STAGE"',
			'  case "$LESSER_STAGE" in',
			'    dev) export ENVIRONMENT="development" ;;',
			'    staging) export ENVIRONMENT="staging" ;;',
			'    live) export ENVIRONMENT="production" ;;',
			'    *) export ENVIRONMENT="$LESSER_STAGE" ;;',
			'  esac',
			'  export DYNAMODB_TABLE="${APP_SLUG}-${LESSER_STAGE}-main-table"',
			'  LESSER_UP_STAGE="--stage ${LESSER_STAGE}"',
			'fi',
			'./lesser up --app "$APP_SLUG" --base-domain "$BASE_DOMAIN" --aws-profile managed ${LESSER_UP_STAGE} --out /tmp/bootstrap.json',
