...
export class LesserHostStack extends cdk.Stack {
  ...
  return request;
		...
		// FaceTheory ISR HTML store: currently holds the build-time static
		// hydration sidecars (one `<routeId>.json` per shell, generated by
		// web/scripts/build-sidecars.mjs and uploaded by WebSidecarsDeployment
		// below; served at /_facetheory/data/* via the addBehavior below).
		// Future ISR-rendered HTML and any future per-request signed sidecars
		// (createSsrHydrationSidecarStore at /_facetheory/ssr-data/<token>)
		// would share this bucket. AppTheorySsrSite grants the SSR Lambda
		// read/write + auto-wires FACETHEORY_ISR_BUCKET / FACETHEORY_ISR_PREFIX
		// for that future opt-in.
