...
1. **Overview** — Fleet link (HomeIcon), Cost & billing (DollarSignIcon), Trust (CheckCircleIcon)
...
3. **Agents** — Souls link (UsersIcon)
4. **Settings** — Account (SettingsIcon) only

### Footer (`.portal-shell__sidebar-footer`)

- **Operator console button** (conditional, above user chip):
...
6. **Souls numeric badge**: The design fixture shows a numeric badge ("2") next to Souls. This is deliberately deferred — M2 has no real `requested`/`in_review` count to surface. Faking a count would be dishonest. Future milestones (M13/M14) that wire actual agent-state counts should add the badge at that point.

## Validation

- Web lint: PASS (0 errors)
- Web typecheck: PASS (0 errors, 0 warnings)
- Web test: PASS (151 tests, 20 files)
...
- Status color mapping: `ok/active/running/provisioned` → ok (green), `warning` → warning (amber), `error/failed` → error (red), `pending/in_progress/creating/provisioning` → accent (gold)
...
Full source: `web/src/lib/components/PortalShell.svelte` (~970 lines)
Tests: `web/src/lib/components/PortalShell.svelte.test.ts` (20 tests)
