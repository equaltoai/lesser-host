...
for f in "${adr}" "${soul}" "${roadmap}" "${controls}"; do
  ...
  forbid_pattern "${f}" 'no (explicit )?retention policy|without (a[n]? )?retention policy|retained? indefinitely|indefinite retention|unbounded retention|retention (is )?(optional|not (required|mandatory|enforced|needed|necessary))|optional retention|skip retention|best[- ]effort retention|keep (content|messages?|mailbox items?) forever|never purge|not (required|mandatory|enforced) to (retain|purge|expire)|(retention|purge|expir(y|ation)).*(not|no) (required|mandatory|enforced|needed)|may (be )?(retained|stored|kept) (indefinitely|forever|without limit)|without (any )?(retention|purging|expiry)|no (requirement|need|obligation) to (retain|purge)|retention (may|can) be (skipped|omitted|avoided|bypassed|waived)|content (may|can) persist (forever|indefinitely|without limit)|(retention|purge).*(not|never|no) (applied?|enforced|implemented)' 'insecure retention semantics'
  forbid_pattern "${f}" 'unencrypted|without encryption|do not encrypt|skip encryption|encryption (is )?(optional|not (required|mandatory|enforced|needed|necessary))|optional encryption|stored? in plaintext|plaintext (mailbox|message|content|body)|not (required|mandatory|enforced) to encrypt|may (be )?(stored|sent|kept|persisted) (unencrypted|in (the )?clear|without encryption)|(stored?|content|messages?|mailbox).*(without|not|no|never) encrypt|encrypt.*(may|can) be (skipped|omitted|avoided|bypassed|waived)|(at rest|in transit).*(not|no|without) encrypt|no (requirement|need|obligation) (for|to) encrypt' 'insecure encryption semantics'
  forbid_pattern "${f}" 'without access[- ]audit|skip access[- ]audit|no access[- ]audit|access audit can be skipped|audit logging (is )?(optional|not (required|mandatory|enforced|needed|necessary))|optional audit|unaudited (read|content|state|mutation)|read(s)? without audit|audit (is )?not (required|mandatory|enforced|needed|necessary)|not (required|mandatory|enforced) to audit|audit (logging )?(may|can) be (skipped|omitted|avoided|bypassed|waived)|without (any )?(audit|auditing|audit trail)|no (requirement|need|obligation) (for|to) audit|(access|content|state|mutation).*(may|can) (be|go) unaudited|audit trail.*(not|no|never) (required?|mandatory|enforced)' 'insecure access-audit semantics'
  forbid_pattern "${f}" 'list endpoints return (full|complete|raw).*bod(y|ies)|full bod(y|ies).*list endpoints|include full bod(y|ies).*list|list (responses|payloads|results) include (full|complete|raw).*(message|content|bod(y|ies))|complete (message|content|bod(y|ies)).*list (responses|payloads|results)|list (responses?|payloads?|endpoints?|results?).*(may|can) include (full|complete|raw|entire).*(bod(y|ies)|message|content)|(full|complete|raw|entire) (bod(y|ies)|message|content).*(may|can) (be )?included in list|list.*not required to redact|redaction.*not (required|mandatory|enforced)|redaction (may|can) be (skipped|omitted|avoided)|list (may|can) return unredacted|unredacted.*list' 'full content in list semantics'
  forbid_pattern "${f}" 'raw instance keys are stored|store raw (instance )?keys|log raw (instance )?keys|return raw (instance )?keys|plaintext (instance[- ]?)?key fallback|instance[- ]auth falls back to plaintext|accept raw (instance )?keys from storage|instance keys? (may|can) be (stored|kept|persisted|retained) (in|as) (plaintext|raw)|(may|can) store (raw|plaintext) (instance )?keys?|keys? (may|can) be (stored|kept|persisted) without hash|hash.*not (required|mandatory|enforced|needed)|not (required|mandatory|enforced) to hash (instance )?keys?|without hash.*instance (auth|key)|instance (auth|key).*without hash|(may|can) accept (raw|plaintext) (instance )?keys?' 'raw-key retention semantics'
  forbid_pattern "${f}" 'cross-tenant (mailbox )?(search|analytics) is allowed|allow(s|ed)? cross-tenant (mailbox )?(search|analytics)|global mailbox (search|analytics) (is )?allowed|search (mailbox )?content across tenants|cross-tenant.*not (prohibited|forbidden|restricted|prevented|blocked)|(may|can) (search|query|access) (mailbox )?(content|data|items?) across tenants|tenant isolation.*not (required|mandatory|enforced|needed|necessary)|not (required|mandatory|enforced) to isolate tenants?|tenant boundar(y|ies).*(may|can) be crossed|cross-tenant (mailbox )?(search|analytics|access).*not (prohibited|forbidden|restricted)' 'cross-tenant mailbox query semantics'
  forbid_pattern "${f}" 'body-owned mailbox storage is allowed|body (owns|stores|persists) mailbox truth|body (may|can) own mailbox (storage|truth|state)|mailbox (storage|truth|state).*(may|can) be body-owned|body.*primary mailbox authority|mailbox authority.*belongs to body|body (may|can) (be|act as) (the )?mailbox (authority|owner)|not (required|mandatory) for host to own mailbox' 'body-owned mailbox authority semantics'
done
