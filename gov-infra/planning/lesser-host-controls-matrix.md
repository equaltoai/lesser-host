...
| Security | T-COMP-001 | C-CDN-COMP | CDN CloudFront composition lock | Enforce CloudFront distribution composition: required behaviors present (API, trust, SSR hydration), SPA rewrite function present, response headers policy applied with correct security headers, OAC origins configured for S3 buckets. SEC-6 enforces no inline HTML; SEC-8 enforces distribution shape. | `bash gov-infra/verifiers/sec/cloudfront-composition.sh` (SEC-8); `bash gov-infra/verifiers/sec/web-html-inline-absence.sh` (SEC-6) | `gov-infra/evidence/SEC-8-output.log`; `gov-infra/evidence/SEC-6-output.log` |
...
| Security | T-SUPPLY-001 | C-SUPPLY-LOCK | Supply-chain release verification change-lock | Enforce preservation of consumer release verification for lesser/body artifacts. SEC-10 change-lock verifier diffs current code against a known-good base ref; any change to checksum verification, release manifest handling, artifact download, or provisioning deploy path must be intentional and reviewed. | `bash gov-infra/verifiers/sec/release-verification-preservation.sh` (SEC-10) | `gov-infra/evidence/SEC-10-output.log` |
