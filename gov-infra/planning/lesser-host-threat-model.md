...
| T-PRUNE-RAWSDK-001 | Deploy-time image pruning drifts into unguarded raw-SDK destructive behavior | The deploy-time pruner (`internal/microvmversions` + `scripts/hosted-genesis-microvm-prune`) is a sanctioned raw-SDK destructive consumer of `aws-sdk-go-v2/service/lambdamicrovms` (a direct dependency that deletes image versions at deploy time). A regression could widen deletion beyond the keep-N/active-pin/state-filter guards, prune the wrong stage's image, silently skip pruning on image-resolution failure (treating name drift as a first deploy), or fail open when the image cannot be brought under the 50-version cap. | QUA-1 CON-2 | `go test ./internal/microvmversions ./scripts/hosted-genesis-microvm-prune` (QUA-1); golangci-lint (CON-2) |
...
- **Sanctioned raw-SDK destructive consumers:** `internal/microvmversions` (keep-N pruning client) and `scripts/hosted-genesis-microvm-prune` (deploy-time CLI) are the intentional destructive consumer of `aws-sdk-go-v2/service/lambdamicrovms` — see `T-PRUNE-RAWSDK-001` and its controls-matrix record (issue #1052, PR #1055). It exists because `AWS::Lambda::MicrovmImage` hard-caps an image at 50 versions with no retention knob. The next import review of any raw-SDK destructive consumer should treat this as the known-good precedent: bounded by keep-N over live versions + active-version pin + settled-state filter, serialized deletes with settle-waits, stage-isolated naming, and fail-closed on cap/headroom/image-resolution failures.
