...
| T-HG-LEGACY-DECL-001 | Fresh hosted-genesis session silently routes to the legacy declaration builder | A fresh M11/Ptah hosted-genesis session runs with a missing/unknown declaration-contract selection and silently selects the legacy v1 declaration builder, producing `invalid_produced_declarations`/`boundaries.required`/`restart_soul_bootstrap` against the five-body contract the deployment intends. Live validation drifts away from the Ptah prototype contract and owners are told to restart when the real fault is deployment configuration. | C-HG-DECL-CONTRACT SEC-15 | `bash gov-infra/verifiers/sec/hosted-genesis-declaration-contract.sh` (SEC-15) |
