...
| THR-5 | Supply-chain compromise | Malicious dependency, compromised GitHub Action, unsafe install script, or over-broad Docker asset context executes or is staged in CI/CodeBuild/local build infrastructure and exfiltrates secrets, retains operator-local material in a build cache, or ships backdoors. | SEC-3 SEC-2 SEC-1 COM-2 QUA-2 | `bash gov-infra/verifiers/gov-verify-rubric.sh` (SEC-3/2/1, COM-2, QUA-2) |
