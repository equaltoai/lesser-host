...
| T-HG-PROVIDER-STALL-001 | Hosted-genesis provider call stalls or MicroVM dies without observable or durable terminal truth | OpenAI/Anthropic SDK retries outlive a coherent bounded HTTP/whole-call/workload/MicroVM envelope, a stream goes idle without a heartbeat, content is exposed while diagnosing deltas/declarations, a `provider_timeout` retry invokes a stale pre-deploy runtime or consumes retry/debit before fresh-runtime preparation, or AppTheory reports a terminated/failed/max-duration MicroVM while Host leaves the accepted turn `in_progress`. A client-side recovery nudge or unguarded write could duplicate work or overwrite a winning completion. | C-HG-PROVIDER-TELEMETRY SEC-16 | `bash gov-infra/verifiers/sec/hosted-genesis-provider-telemetry.sh` (SEC-16) |
