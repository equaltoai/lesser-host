...
| T-HG-CANDIDATE-INTEGRITY-001 | Hosted Genesis review or publication diverges from accepted typed candidate truth | A provider tool submission can target a stale revision/hash, replay another turn's call identity, smuggle unknown fields, lose accepted sections across failure/recovery, or cause review/affirmation/finalization to bind different bytes. A retained transcript extractor, phrase-only affirmation, post-affirmation provider request, fallback lane, or unguarded candidate/projection write could publish declarations the owner never reviewed. | C-HG-TYPED-CANDIDATE SEC-16 | `bash gov-infra/verifiers/sec/hosted-genesis-provider-telemetry.sh` (SEC-16) |
