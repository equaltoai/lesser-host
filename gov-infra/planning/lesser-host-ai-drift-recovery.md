# AI/Agent Drift: Failure Modes + Recovery (lesser-host)
...
- **Scope dodging:** only validating the root module while nested modules or frontends are broken.
...
- Maintain public API contract parity.
- Keep maintainability budgets (file size/complexity) and convergence plans current.

## Recovery playbook
1. Re-run the full rubric surface and capture outputs under `gov-infra/evidence/`.
