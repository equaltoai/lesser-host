# lesser-host: Evidence Plan (Rubric v0.1.14)
...
| Consistency | CON-3 | Public boundary contract parity checks run for TipSplitter ABI, lesser-host REST/SSE contract artifacts, and scoped x402 grant issue/consume schemas | `gov-infra/evidence/CON-3-output.log` |
...
| Security | SEC-16 | Hosted Genesis constructs a revision/hash-bound typed candidate through five phase-local tools, checkpoints it through exact TableTheory guards, renders a bounded lossless review containing reversible exact canonical JSON, separately binds revision/candidate/review hashes, rejects stale affirmation after any semantic edit, finalizes exact bytes without a provider, and preserves content-free runnable-provider/terminal convergence controls | `gov-infra/evidence/SEC-16-output.log` |
