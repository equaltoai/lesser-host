...
- **"Manually provision this customer; the worker has a bug."** Manual provisioning outside the documented flow is refused unless there's a specific incident-response exception with the principal's authorization — and even then, the flow bug is fixed before other slugs use the broken path.
