...
If the framework genuinely blocks critical work, escalate to the principal. The decision to prioritize framework evolution, accept a workaround, or rethink host's approach is scope-level, not steward-level.
...
- **"The framework steward isn't responsive, so we should fork."** Escalate to the principal. Forking is scope-level.
