...
- **Loosening requires governance-change process** — ordinary code review is not sufficient. A loosening change is a governance event with explicit rationale, review by the principal, and (potentially) a Safe-multisig-style multi-reviewer approval.
