...
3. The hosted-genesis SQS queue, if present, is non-authoritative operator/backfill/janitor transport. DLQ backlog or AI-worker outage must not change what status reads or finalize preflight decide once a `HostedGenesisSession` exists.
...
- Treat hosted-genesis queue/DLQ messages as hints for backfill or cleanup only. Never replay a message without checking the session `version`, current `status`, registration id, agent id, instance slug, and turn/idempotency ledger.
